...
from app.learning.aliases import learn_from_skill_diff


def test_learn_skill_correction_creates_alias(db):
    learn_from_skill_diff(
        db, raw_skills=["GoLang", "reactjs"], edited_skills=["Go", "React"])
    db.commit()
    golang = db.query(EntityAlias).filter_by(
        alias="golang", kind="skill").one()
...
def test_learn_skill_records_case_only_correction(db):
    ...
    learn_from_skill_diff(
        db, raw_skills=["FastAPI"], edited_skills=["fastapi"])
    db.commit()
